	...
	namespace Platform {
		namespace Elements {
			...
			public:
				...
				static Dai<Muon^, Charm^>^ Tethys;
				static Edge<Nucleon^, Charm^, Down^>^ Laodemeia;
				static Edge<Down^, Muon^, Nucleon^>^ Anthe;
				...
				Charm(Down^ Beta) {
					...
					if (Tethys == nullptr) Tethys = gcnew Dai<Muon^, Charm^>();
					...
					if (Anthe == nullptr) Anthe = gcnew Edge<Down^, Muon^, Nucleon^>(Beta, Beta[0]->I[2]->L, Beta[0]->L);
				}
			};
		}
	}
