	...
	namespace Platform {
		namespace Elements {
			...
			public:
				...
				BigInteger Evidence() {
					return BigInteger::ModPow(L->Phi->N->Identity, N->Mu.Value->Mu.Key->Phi->N->Ring, R->Rod->Barn);
				}

				BigInteger Testify(BigInteger Phi) {
					return BigInteger::ModPow(Phi, N->Mu.Value->Mu.Key->Phi->N->Ring, R->Cone->Barn);
				}
			};
		}
	}
